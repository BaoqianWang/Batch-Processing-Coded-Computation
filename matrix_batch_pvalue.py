...
for k in range(num_iteration):
	...
	if rank==1:
		...
		batch_size=int(np.ceil(matrixRecv.shape[0]/pValue))
		for j in range(pValue-1):
			waitT1=MPI.Wtime()
		...
		waitT3=MPI.Wtime()
		matrixResFinal=np.matmul(matrixRecv[(pValue-1)*batch_size:,:],recv_x)
		waitT4=MPI.Wtime()

...
for i in range(len(worker_load)):
    ...
    print(worker_load[i][0], worker_load[i][1])
    for j in range(worker_load[i][0], worker_load[i][1]):
        selection_indexes = lt_code.list_indexes[j]
        load_worker[j-worker_load[i][0],:] = np.sum(A[selection_indexes, :],axis=0)
encoding_end = time.time()
print('Encoding time', encoding_end - encoding_start)
...
#     for j in range(worker_load[i][0], worker_load[i][1]):
#         selection_indexes = lt_code.list_indexes[j]
#         load_worker[j,:] = np.sum(A[selection_indexes, :],axis=0)
    #print(load_worker)
